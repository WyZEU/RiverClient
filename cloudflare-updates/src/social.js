...
/*
  Offered by the in-game client and echoed back, which completes the handshake and gates
  the route. Named for what rides it rather than the version of the client, so it can
  carry more than messages later without a second connection.
*/
export const SOCIAL_PROTOCOL = "river.social.v1";
...
export class Social {
  constructor(state, env) {
    ...
    /*
      Connected players are not tracked in a map here.

      Hibernation drops this object from memory between messages while keeping its
      sockets, so anything held in a field would be gone by the time a message needed
      delivering and every push would silently reach nobody. The uuid is attached to the
      socket itself instead, which survives eviction, and push() reads it back off.
    */
    // Answered by the runtime without waking this object, so an idle client costs nothing.
    try {
  ...
  /** Delivers to every socket that player has open, across however many machines. */
  push(uuid, payload) {
    if (!uuid) return;
    ...
    for (const socket of sockets) {
      ...
      if (attached?.uuid !== uuid) continue;
      try { socket.send(text); } catch {}
    }
  }

  /*
    Clients are not asked to say anything: the only direction that carries meaning is
    server to client. A ping is answered by hand for runtimes without auto-response.
  */
  ...
  async fetch(request) {
    ...
    /*
      A WebSocket handshake can also carry the token as a subprotocol. That is not a
      preference, it is the only option the launcher has: Node cannot set headers on a
      handshake, and a session token has no business in a URL where it would be logged.
      The in-game client is Java, can set headers, and uses Authorization like every
      other route here.
    */
    const offeredProtocols = (request.headers.get("Sec-WebSocket-Protocol") || "").split(",").map((value) => value.trim());
    const protocolToken = offeredProtocols.find((value) => value.startsWith("bearer."))?.slice("bearer.".length) || "";
    const token = (request.headers.get("Authorization") || "").replace(/^Bearer\s+/i, "") || protocolToken || String(body.token || "");
    ...
    switch (route) {
      ...
      /*
        A friend request or a direct message arrives here rather than being found by the
        next poll. The in-game client refreshed every ten seconds, so a message could sit
        unseen that long and the poll ran regardless of whether anything had happened.

        Authenticated by the same bearer token as every other route above: unlike Node,
        Java can set headers on a WebSocket handshake, so the session travels properly
        rather than being smuggled through the URL.
      */
      case "/ws": {
        ...
        if (!offered.includes(SOCIAL_PROTOCOL)) return fail("Not found.", 404);
        const pair = new WebSocketPair();
        const [client, server] = Object.values(pair);
        this.state.acceptWebSocket(server);
        // The identity lives on the socket, not in a field - see the constructor.
        try { server.serializeAttachment({ uuid: session.uuid, name: session.name }); } catch {}
        return new Response(null, {
          status: 101,
          webSocket: client,
          headers: { "Sec-WebSocket-Protocol": SOCIAL_PROTOCOL }
        });
      }
      default:
