...
/*
  Every counter has a unique twin. The dashboard shows the unique one everywhere except
  update checks, because "how many people" is the question being asked of all of them -
  a raw count answers "how much did the software talk to me", which is a different and
  much less interesting thing. Checks keep both, because there the raw figure is the
  point: it is the traffic number.
*/
const EMPTY_DAY = {
  installs: 0, installsUnique: 0,
  checks: 0, checksUnique: 0,
  launches: 0, launchesUnique: 0,
  failures: 0, failuresUnique: 0
};
...
export class Metrics {
  ...
  */
  ...
  async record(kind, request, extra = {}) {
    ...
    // Added after the first records were written, so they need the same treatment.
    for (const name of ["versionsUnique", "mcUnique", "reasonsUnique"]) {
      if (!day[name]) day[name] = {};
      if (!totals[name]) totals[name] = {};
    }
    for (const name of ["launchesUnique", "failuresUnique", "installsUnique", "checksUnique"]) {
      if (typeof day[name] !== "number") day[name] = 0;
      if (typeof totals[name] !== "number") totals[name] = 0;
    }

    /*
      Counts one person once against a breakdown rather than once per request. A launcher
      polling all day used to pile every poll onto its own version, which is how the
      version chart came to read in the thousands off a handful of people.
    */
    const bumpUnique = async (bucket, key) => {
      if (!key) return;
      const seen = await this.firstSightings(bucket + ":" + key, id, date, now);
      if (seen.today) bump(day[bucket + "Unique"], key);
      if (seen.allTime) bump(totals[bucket + "Unique"], key);
    };
    ...
    */
    ...
    if (kind === "install" || kind === "check") {
      ...
      if (kind === "check") {
        ...
        await bumpUnique("versions", version);
    ...
    if (kind === "launch" || kind === "failure") {
      ...
      const uniqueField = field + "Unique";
      const seen = await this.firstSightings(kind, id, date, now);
      day[field] = (day[field] || 0) + 1;
      totals[field] = (totals[field] || 0) + 1;
      if (seen.today) day[uniqueField] = (day[uniqueField] || 0) + 1;
      ...
      */
      if (kind === "launch") {
        ...
        await bumpUnique("mc", tag(extra.mcVersion, 16));
      ...
      if (kind === "failure") {
        ...
        await bumpUnique("reasons", reason);
      }
    }
