...
// Everything this window can reach is declared in preload-logs.js. It has no Node
// access of its own.
const bridge = window.riverLogs;
...
function LogsApp() {
  ...
  useEffect(() => {
    // The bridge unwraps the IPC event and hands over the payload alone, so this takes
    // one argument. Written as (event, payload) it silently received undefined for every
    // line and the window stayed on "Waiting for output" forever.
    const onLog = (payload) => {
    ...
    const offLog = bridge.onLog(onLog);
    const offCleared = bridge.onCleared(onCleared);
    bridge.ready().catch(() => {});
    return () => {
      offLog();
      offCleared();
  ...
  useEffect(() => {
    const onMax = (value) => setMaximized(Boolean(value));
    return bridge.onMaximized(onMax);
  }, []);
  ...
  const copyVisible = useCallback(() => {
    bridge.copy(visible.map((l) => l.text).join("\n")).catch(() => {});
  }, [visible]);
    ...
    <div className="logs-shell">
      ...
      <div className="logs-toolbar">
        ...
        <div className="logs-actions">
          ...
          <button type="button" onClick={() => bridge.openFolder().catch(() => {})} title="Open the log folder"><FolderOpen size={13} /> Folder</button>
