...
import {
  AlertTriangle, Download, PowerOff, FileText, Copy, Loader2, ShieldCheck, ChevronDown, Upload, Check
} from "lucide-react";
...
export function CrashReport({ status, refresh, notify }) {
  ...
  // Reset per report, so a later crash can be sent even though this one already was.
  ...
  return (
    <section className="rounded-lg border border-destructive/40 bg-card">
      ...
      <div className="flex flex-wrap items-center gap-2 p-4">
        ...
        {/*
          Nothing about a crash is sent unless this is pressed. The label says where it
          goes rather than "Send report", because a button that uploads your log should
          say so before you press it, not after.
        */}
        <Button
          size="sm"
          variant="ghost"
          disabled={Boolean(busy) || sent === "ok"}
          title="Uploads this crash report so it can be looked at. Your username, file paths and any sign-in tokens are removed first."
          onClick={() => run(
            "send",
            async () => {
              const res = await api()?.uploadCrashReport?.({ path: analysis.report?.path });
              if (res?.ok) setSent("ok");
              return res;
            },
            "Crash report sent. Thanks."
          )}
        >
          {busy === "send" ? <Loader2 className="size-3.5 animate-spin" />
            : sent === "ok" ? <Check className="size-3.5" />
            : <Upload className="size-3.5" />}
          {sent === "ok" ? "Report sent" : "Send this report"}
        </Button>
