...
function StorageSection({ notify }) {
  ...
  return (
    <Section title="Storage">
      ...
      <Field label="Instance files" hint="Mods, worlds, packs, shaders">
        <span className="font-mono text-xs text-muted-foreground">{formatBytes(info?.instanceSize)}</span>
      </Field>

      <Field label="Cache" hint="Mod metadata and thumbnails">
    ...
    <Page title="Settings" description={`River Client ${status?.version || ""}`.trim()}>
      <div className="space-y-6">
        ...
        <Section title="Game">
          ...
          {/*
            These two were the reason River looked slower than other launchers: a fresh
            profile ships capped at 120 with vsync on, and River had a maxFps setting that
            nothing ever read. They are written into the instance's options.txt at launch.
          */}
          <Field
            label="Frame rate limit"
            hint="Set once when an instance is new. Change it in the game and River keeps yours."
          >
            <Select
              value={String(settings.maxFps ?? 165)}
              onValueChange={(v) => patch({ maxFps: Number(v) })}
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="360">360 fps</SelectItem>
              </SelectContent>
            </Select>
          </Field>

          <Field label="VSync" hint="Caps the game at your monitor's refresh rate">
            <Switch checked={Boolean(settings.vsync)} onCheckedChange={(v) => patch({ vsync: v })} />
          </Field>
          ...
          <Field label="Java path" hint="Blank uses the bundled runtime">
            <Input
              className="w-64"
        ...
        <Section title="Launcher">
          <Field label="Game log window" hint="Separate window at launch">
            <Switch
          ...
          <Field label="Keep launcher open">
            <Switch checked={Boolean(settings.keepLauncherOpen)} onCheckedChange={(v) => patch({ keepLauncherOpen: v })} />
          </Field>

          <Field label="Close to tray" hint="Minimize instead of quit">
            <Switch checked={settings.closeToTray !== false} onCheckedChange={(v) => patch({ closeToTray: v })} />
          </Field>

          <Field label="Message notifications" hint="Only when the game is closed">
            <Switch
              checked={settings.dmNotifications !== false}
              onCheckedChange={(v) => patch({ dmNotifications: v })}
            />
          </Field>
          ...
          <Field label="Automatic updates" hint="On startup">
        ...
        <Section title="Maintenance">
          ...
          <Field
            ...
            hint="Tester builds, if you're allowlisted"
          >
          ...
          <Field label="Tutorial" hint="Replay the tour">
          ...
          <Field label="Repair all instances" hint="Re-verify game files">
            <Button variant="outline" size="sm" disabled={busy} onClick={async () => {
              setBusy(true);
              try {
