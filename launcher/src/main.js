...
const { app, BrowserWindow, ipcMain, shell, dialog, screen, nativeImage, globalShortcut, clipboard, Notification } = require("electron");
const { spawn } = require("node:child_process");
const crypto = require("node:crypto");
// Shared with scripts/publish-update.js on purpose: if the two ends chunked differently
// no hash would ever match and every patch would silently fall back to a full download.
const { chunkBuffer } = require("../scripts/blockmap.js");
...
/*
  A stable, anonymous id for this installation.

  Usage counting identified an install by hashing its address together with its
  User-Agent, and the User-Agent above carries the launcher version. Every release
  therefore minted a brand new "person": somebody who followed 0.1.8.1 through 0.1.8.7
  was counted as seven of them. A changing address did the same thing on its own, which
  is routine on any ISP that forces a daily reconnect.

  This is a random value generated once and kept on disk. It is not derived from the
  hardware, the account or the address, so it says nothing about who is running River -
  it is strictly less identifying than the address it replaces, and unlike the address it
  actually stays put. It lives in its own file rather than settings.json so that clearing
  settings does not silently turn one person back into two.
*/
...
function readInstallId() {
  ...
  try {
    const stored = JSON.parse(fs.readFileSync(installIdPath(), "utf8"));
    const existing = String(stored?.id || "").trim();
  ...
  try {
    ...
    fs.writeFileSync(installIdPath(), JSON.stringify({ id: created, createdAt: Date.now() }, null, 2));
  } catch {
    // An install that cannot write the file still runs; it just goes back to being
    // counted by address, which is what happened before this existed.
  }
  cachedInstallId = created;
  return cachedInstallId;
}

/** Headers for the requests the server counts, so one install reads as one install. */
function metricsHeaders(extra = {}) {
  const headers = { "User-Agent": launcherUserAgent, ...extra };
  const id = readInstallId();
  if (id) headers["X-River-Install"] = id;
  return headers;
}
...
/*
  Every Minecraft version River ships an in-game build for, newest first. One source
  tree builds all of them through Stonecutter, so adding one means adding a
  versions/<mc>/gradle.properties entry rather than forking the mod.

  Keep in step with site.mcVersions on the website, which advertises this list.
*/
const SUPPORTED_MC_VERSIONS = ["26.2", "26.1.2", "1.21.11", "1.21.8", "1.21.7", "1.21.6", "1.21.5", "1.21.4", "1.21.3", "1.21.2"];
/*
  Newest first for display, but the default is pinned rather than taken from the front
  of the list: 26.x is new enough that it should be something you pick, not something
  every existing install is silently moved onto. It also decides which bundled jar
  carries no `+<mc>` suffix, so moving it would rename a shipped file.
*/
const DEFAULT_MC_VERSION = "1.21.11";

/*
  26.1 and later are built against Java 25 and their class files will not load on 21.
  Everything before them stays on 21, so the requirement follows the version rather
  than being one number for the whole launcher.
*/
function requiredJavaMajor(mcVersion) {
  return String(mcVersion || DEFAULT_MC_VERSION).startsWith("1.") ? 21 : 25;
}
...
function resolveRiverRuntimeJars(mcVersion = DEFAULT_MC_VERSION) {
  ...
  const clientLibDirs = [
    ...
    // Stonecutter writes each target to versions/<mc>/build/libs. The clientcore-<mc>/
    // path is the pre-Stonecutter layout, kept so an older checkout still resolves.
    ...(root ? SUPPORTED_MC_VERSIONS.map((v) => path.join(root, "versions", v, "build", "libs")) : []),
...
/**
 * Applies River's frame rate settings to an instance's options.txt.
 *
 * maxFps existed as a setting with a default of 240 and was never read by anything, so
 * a instance sat on whatever its own options.txt said - which is 120 with vsync on for a
 * fresh profile, and looks like River being slow when it is really just capped.
 *
 * Only the two keys are touched, and only when they differ, so everything else the
 * player set in game is left exactly as it was. `vsync: null` means "leave it alone".
 */
/**
 * Applies River's frame rate settings to an instance, without overriding the player.
 *
 * This used to write both values on every single launch, which meant River quietly undid
 * whatever you set in the game's own video settings, every time, forever. Change the frame
 * limit in Minecraft, quit, launch again, and it was back to whatever the launcher held.
 *
 * So River now only writes a value it is still responsible for. It remembers what it last
 * wrote; if the file still says that, the setting is River's to change, and it applies the
 * launcher's current choice. If the file says something else, the player changed it in the
 * game and it is theirs - River leaves it alone and adopts their value as the new baseline,
 * so it never fights them again.
 *
 * The first time round there is no baseline, so the launcher's value is applied once and
 * recorded. That keeps the original point of the setting working for a fresh instance.
 */
function applyFrameRateOptions(instancePath, { maxFps, vsync }) {
  const filePath = path.join(instancePath, "options.txt");
  if (!fs.existsSync(filePath)) return [];
  const changed = [];
  try {
    const applied = readInstanceMeta(instancePath) || {};
    ...
    const setKey = (key, value) => {
      ...
      if (i === -1) { lines.push(want); changed.push(want); return; }
      if (lines[i] !== want) { changed.push(`${lines[i]} -> ${want}`); lines[i] = want; }
    };

    // Returns the value to record as the new baseline, whoever last set it.
    const reconcile = (key, wanted, lastApplied) => {
      const current = readKey(key);
      const haveBaseline = lastApplied !== undefined && lastApplied !== null;

      /*
        No baseline and the game already has a value: this is an instance from before
        River started tracking who set what. Its value is the player's, arrived at over
        however long they have been using it, so it is adopted rather than overwritten.
        Writing the launcher's value here would stomp everybody's setting exactly once on
        upgrade, which is the very thing this change exists to stop.
      */
      if (!haveBaseline && current !== null) {
        emit("launcher:log", `[launcher] Keeping the ${key} of ${current} this instance already had.`);
        return current;
      }
      if (haveBaseline && current !== null && current !== String(lastApplied)) {
        // The player changed this in game since River last wrote it. Theirs now.
        emit("launcher:log", `[launcher] Keeping your own ${key} of ${current} from the game's settings.`);
        return current;
      }
      setKey(key, wanted);
    ...
    const nextMeta = {};
    if (Number.isFinite(Number(maxFps))) {
      nextMeta.appliedMaxFps = reconcile("maxFps", Number(maxFps), applied.appliedMaxFps);
    }
    if (vsync !== null && vsync !== undefined) {
      nextMeta.appliedVsync = reconcile("enableVsync", vsync ? "true" : "false", applied.appliedVsync);
    }

    if (changed.length) fs.writeFileSync(filePath, lines.join("\n"));
    if (Object.keys(nextMeta).length) writeInstanceMeta(instancePath, nextMeta);
  } catch (error) {
    emit("launcher:log", `[launcher] Could not apply frame rate options: ${error.message}`);
  }
  return changed;
}
...
/**
 * Turns whatever a server icon arrives as into something an <img> can show.
 *
 * servers.dat gives bare base64; mcsrvstat already returns a full data URI. Both
 * end up here so the callers do not have to care which one they got.
 */
function normalizeServerIcon(value) {
  ...
  // Strip whitespace/newlines the NBT string may carry before validating.
...
function parseServersDat(filePath) {
  try {
    ...
    // servers.dat stores the icon as bare base64 PNG bytes with no data: prefix,
    // so feeding it straight to an <img src> renders nothing. Most saved servers
    // have one, which is why nearly every row came up iconless.
    return root.servers.map(s => ({
      name: s.name || "",
      ip: s.ip || "",
      icon: normalizeServerIcon(s.icon)
    }));
  } catch { return []; }
}
...
/**
 * Turns the message a failed launch showed into a short label for the stats.
 *
 * Two things it has to get right, both learned from labels that turned out to be useless.
 *
 * The interesting half of a message is usually at the end, not the start. "Minecraft
 * preparation failed: Download timed out" and "Minecraft preparation failed: checksum did
 * not match" are entirely different problems that both begin with the same four words, so
 * taking the first few words of the whole string produced one bucket that could not be
 * acted on. The generic prefix before the first colon is dropped.
 *
 * A leading number is a count, not a cause. "3 confirmed mod compatibility issues" is the
 * same failure as "1 confirmed mod compatibility issue", and keeping the digit split one
 * problem across a bar per count. Only a leading number is dropped, because a number
 * further in is usually the useful part - an HTTP status, for one.
 */
function shortFailureReason(raw) {
  let text = String(raw || "");
  const colon = text.indexOf(": ");
  // Only strip a prefix that is short enough to be a generic heading, so a message that
  // happens to contain a colon does not lose its beginning.
  ...
  /*
    Filler carries no signal and pushes the words that do past the cut, and a plural is the
    same failure as its singular - "1 ... issue" against "3 ... issues" split one cause in
    two the moment the label grew long enough to reach the noun.
  */
  const filler = new Set(["a", "an", "the", "is", "are", "was", "be", "been", "must", "and",
    "or", "of", "to", "in", "on", "for", "with", "from", "that", "this", "it", "your", "you"]);
  const key = words
...
/*
  Tells the update server that a launch happened, or that one failed.

  This is the only way either is visible. Presence needs a signed-in profile and a game
  that got as far as starting, so a launch that fails sends nothing at all - failures were
  previously invisible unless somebody said something in Discord. The body carries the
  Minecraft version, the launcher version and, for a failure, a short reason. There is no
  account, no machine id and nothing that ties two launches to the same person.

  Deliberately not awaited anywhere it is called: a launch must not wait on, or be failed
  by, a counter.
*/
function reportLaunchMetric(event, extra = {}) {
  const body = JSON.stringify({
    event,
    mcVersion: String(extra.mcVersion || "").slice(0, 16),
    launcherVersion: app.getVersion(),
    /*
      Free text becomes a chart label, so it is cut down to a short stable phrase rather
      than sent as a whole error message with paths and ids in it.

      Numbers are dropped rather than kept. Several of these messages start with a count -
      "3 confirmed mod compatibility issues must be fixed" - and keeping it split one cause
      into a separate bar per count, so the most common reason for a failed launch looked
      like three uncommon ones.
    */
    reason: shortFailureReason(extra.reason)
  });

  fetch("https://updates.riverclient.xyz/metrics/launch", {
    method: "POST",
    headers: metricsHeaders({ "Content-Type": "application/json" }),
    body
  }).catch(() => {});
}
...
// clientcore's fabric.mod.json declares the loader it needs, and the 26.x builds ask for
// 0.19.5; running an older one leaves those instances on a loader clientcore already
// declares itself incompatible with. Loader versions aren't tied to a Minecraft version,
// so one current version covers every instance.
const fabricLoaderVersion = "0.19.5";
...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    // Same as --background in the renderer, so opening the window does not flash one
    // colour before the document paints another.
...
function normalizeSettings(value) {
  ...
  /*
    0 (uncapped) used to be an offered choice. On a real modpack it let one mod render
    thousands of frames a second with no cap and no vsync, and another mod's GL timer
    query degenerated to Infinity under that load and logged an unthrottled error every
    single frame - effectively a hang. 240 is removed from the allowed set going forward
    for the same reason: it is a stand-in "very high" number, not a real target anyone
    picked for a reason, and it carries the identical risk on lighter scenes.
    An instance that still has 0 or 240 in its settings (from before this change, or from
    editing the file directly) is moved to 165 rather than accepted, since accepting it
    silently would mean this exact failure returns for exactly the people who hit it.
  */
  /*
    The allowed list has to contain everything the settings screen offers, or picking one
    of them silently snaps back: 240 was in the dropdown and not in this list, so choosing
    it did nothing and looked like the setting was ignored.

    Unlimited is deliberately still absent. It went in once, and with no cap and no vsync a
    rendering mod's frame timer degenerated and wrote millions of log lines until the game
    hung, with no crash report to show for it.
  */
  const ALLOWED_FPS = [60, 120, 144, 165, 240, 360];
  const requestedFps = Number(merged.maxFps);
  merged.maxFps = ALLOWED_FPS.includes(requestedFps) ? requestedFps : 165;
  merged.vsync = Boolean(merged.vsync);
  ...
  merged.dmNotifications = merged.dmNotifications !== false;
...
/*
  Tears down an RPC client without letting the teardown itself become an error.

  `destroy()` returns a promise, so `destroyDiscordRpcClient(client);` catches nothing:
  the failure happens after the try block has already returned. Discord's transport throws
  "Cannot read properties of null (reading 'write')" whenever it is closed after the socket
  has already gone, which is the normal case when Discord is the side that went away, and
  every one of those landed in the log as an unhandled rejection.
*/
...
function queueDiscordRpcReconnect(delayMs = 4000, reason = "Trying to reconnect to Discord...") {
  ...
  if (discordRpcClient) {
    destroyDiscordRpcClient(discordRpcClient);
  }
...
/*
  Status lookups are cached for a short while and never run more than a handful at a time.

  Both matter. The list can hold sixty saved servers and every one of them used to be
  looked up at once, which the status API rate limits - so the requests failed, and a
  failure was reported as the server being offline. That is how a list of servers that were
  all plainly up came to render as all offline.
*/
const SERVER_STATUS_TTL_MS = 60_000;
const SERVER_STATUS_CONCURRENCY = 5;
const serverStatusCache = new Map();

async function fetchServerStatus(server) {
  const key = String(server.ip || "").toLowerCase();
  const cached = serverStatusCache.get(key);
  if (cached && Date.now() - cached.at < SERVER_STATUS_TTL_MS) {
    return { ...cached.value, name: server.name || cached.value.name, partner: Boolean(server.partner) };
  }
  ...
  try {
    ...
    // Ten seconds, not four. A server the API has not seen before is looked up live, and
    // that regularly takes longer than four - which was being reported as offline.
    ...
    const value = {
      ...
      // True only when the lookup actually succeeded. Without this the UI cannot tell
      // "this server is down" from "River could not find out", and it showed both as down.
      statusKnown: true,
    ...
    serverStatusCache.set(key, { at: Date.now(), value });
    return value;
  } catch (error) {
    emit("launcher:log", `[servers] Could not check ${server.ip}: ${error.message}`);
    // Not cached: a lookup that failed should be retried, not remembered as a result.
    return { ...base, online: false, statusKnown: false };
  }
}

/** Runs [work] over [items] a few at a time, so a long server list cannot burst the API. */
...
function detectExternalMinecraftInstances() {
  ...
  for (const candidate of candidates.concat(multiMcConfiguredInstanceDirs())) {
...
/*
  Where a MultiMC-family launcher actually keeps its instances.

  Prism, MultiMC and PolyMC all let you move the instance folder, and people with a lot
  of instances usually have - onto a second drive, most often. The default paths above
  then find nothing at all, and River reports no instances for someone who has twenty.
  The launcher records the real location in its own config, so read it.
*/
function multiMcConfiguredInstanceDirs() {
  const appData = app.getPath("appData");
  const localAppData = process.env.LOCALAPPDATA || path.join(app.getPath("home"), "AppData", "Local");
  const configs = [
    { file: path.join(appData, "PrismLauncher", "prismlauncher.cfg"), launcher: "Prism Launcher" },
    { file: path.join(localAppData, "PrismLauncher", "prismlauncher.cfg"), launcher: "Prism Launcher" },
    { file: path.join(appData, "PolyMC", "polymc.cfg"), launcher: "PolyMC" },
    { file: path.join(appData, "MultiMC", "multimc.cfg"), launcher: "MultiMC" }
  ];

  const roots = [];
  for (const { file, launcher } of configs) {
    try {
      if (!fs.existsSync(file)) continue;
      const match = fs.readFileSync(file, "utf8").match(/^InstanceDir=(.+)$/m);
      if (!match) continue;
      const raw = match[1].trim();
      if (!raw) continue;
      // The config stores a relative path for a portable install, absolute otherwise.
      const dir = path.isAbsolute(raw) ? raw : path.resolve(path.dirname(file), raw);
      if (fs.existsSync(dir)) roots.push({ name: launcher + " instance", gameDir: dir, launcher });
    } catch {
      // An unreadable config just means we fall back to the default paths.
    }
  }
  return roots;
}
...
/*
  Copies without blocking the main process.

  This used to be cpSync. Copying a mods folder that way is imperceptible; copying saves
  is not - a few large worlds is gigabytes, and every one of those bytes moved with the
  Electron main thread held. The window stops painting, Windows paints "not responding"
  over it, and people force-close a launcher that was working perfectly. That is what
  happened to somebody importing an instance.
*/
async function copyExistingFolderContents(source, target, onProgress) {
...
async function importExternalMinecraftInstance(entry = {}) {
  ...
  for (const folder of ["mods", "resourcepacks", "shaderpacks", "config", "saves"]) {
    /*
      Reported as it goes, because saves can be gigabytes. Without something on screen an
      import of a big world looks exactly like the launcher having hung.
    */
    emitActivity({ title: "Importing instance", detail: "Copying " + folder + "...", current: 0, total: 1 });
    await copyExistingFolderContents(path.join(sourceDir, folder), path.join(targetPath, folder));
  }
  emitActivity({ title: "Importing instance", detail: "Finishing up...", current: 1, total: 1, done: true });
...
ipcMain.handle("launcher:import-external-instance", async (_event, entry) => {
  try {
    return await importExternalMinecraftInstance(entry);
  } catch (error) {
...
async function runFullRepair() {
  ...
  await step("Java", async () => {
    ...
    const needMajor = requiredJavaMajor(version);
    if (current && (!fs.existsSync(current) || javaMajorVersion(current) < needMajor)) {
      writeSettings({ ...readSettings(), javaPath: "" });
      emit("launcher:log", `[fix] Cleared unusable saved Java path: ${current}`);
    }
    let java = resolveUsableJava(readSettings(), needMajor);
    if (!java) {
      emit("launcher:log", `[fix] No Java ${needMajor}+ found. Installing Adoptium JRE ${needMajor}...`);
      const install = await installJavaRuntime(needMajor);
      if (!install.ok || !install.javaPath) throw new Error(install.message || "Java install failed.");
      java = install.javaPath;
      return `Installed Java ${needMajor}`;
    }
    writeSettings({ ...readSettings(), javaPath: java });
    return `Java ${needMajor} ready (${javaMajorVersion(java)})`;
  });
...
/**
 * The player's saved server list, straight out of servers.dat.
 *
 * Only servers the player actually saved appear here. This used to also splice in
 * options.txt's lastServer when it was not already saved, which meant anything you
 * joined once - a friend's temporary e4mc address, a server from a link - showed up
 * in the list as though you had added it. Joining something is not the same as
 * saving it, so that entry is gone.
 *
 * lastServer is still read, but only to sort the server you most recently played to
 * the top. It cannot introduce a row that is not in servers.dat.
 */
ipcMain.handle("launcher:get-recent-servers", async () => {
  ...
  let saved = parseServersDat(datPath)
    .map((srv) => ({
  ...
  // A cap only so a pathological servers.dat cannot fire hundreds of status pings.
  // The old limit of 15 silently hid saved servers from anyone with a longer list.
  const limited = saved.slice(0, 60);
  if (!limited.length) return [];

  return mapWithConcurrency(limited, SERVER_STATUS_CONCURRENCY, fetchServerStatus);
});
...
/*
  Sends a crash report to me, and only ever because the user pressed the button. Nothing
  about a crash leaves the machine on its own.

  The log is scrubbed here as well as on the server. Doing it on the server alone would
  mean the unredacted text crossing the network, and doing it here alone would mean
  trusting whatever arrives; the machine name and any session token in the launch
  arguments should not survive either trip.
*/
function scrubCrashLog(raw) {
  // The argument form is replaced before the JWT form, because its value is usually a JWT
  // and the other order leaves a half-redacted placeholder behind.
  return String(raw || "")
    .replace(/(--(?:accessToken|access_token|clientToken|session|uuid|xuid)[=\s]+)[^\s"']+/gi, "$1[removed]")
    .replace(/eyJ[A-Za-z0-9_-]{8,}\.[A-Za-z0-9_-]{8,}\.[A-Za-z0-9_-]{8,}/g, "[token removed]")
    .replace(/\b(access[_-]?token|refresh[_-]?token|session[_-]?token|id[_-]?token|client[_-]?secret|api[_-]?key)\b(["'\s:=]+)[A-Za-z0-9._~+/-]{8,}/gi, "$1$2[removed]")
    .replace(/\bBearer\s+[A-Za-z0-9._~+/-]{8,}=*/gi, "Bearer [removed]")
    .replace(/([A-Za-z]:\\Users\\)[^\\\r\n"']+/g, "$1[user]")
    .replace(/\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b/g, "[email removed]");
}

ipcMain.handle("launcher:upload-crash-report", async (_event, options = {}) => {
  // Same allow-list as reading one, so this cannot be pointed at an arbitrary file.
  const report = readCrashReport(options?.path);
  if (!report.ok) return report;

  const status = getStatus();
  const payload = {
    log: scrubCrashLog(report.text),
    note: scrubCrashLog(options?.note).slice(0, 2000),
    mcVersion: status.settings.selectedVersion,
    launcherVersion: app.getVersion()
  };

  try {
    const response = await fetch("https://updates.riverclient.xyz/crash-report", {
      method: "POST",
      headers: { "Content-Type": "application/json", "User-Agent": launcherUserAgent },
      body: JSON.stringify(payload)
    });
    if (!response.ok) {
      return { ok: false, message: `Upload failed (${response.status}). Try again, or post the file in Discord.` };
    }
    const result = await response.json().catch(() => ({}));
    return { ok: true, id: result?.id || "", message: "Report sent. Thanks." };
  } catch (error) {
...
ipcMain.handle("launcher:launch-client", async (_event, opts = {}) => {
  ...
  const failLaunch = (message, extra = {}) => {
    /*
      Whatever the caller worked out about the failure is kept, not just the sentence.

      The mod compatibility check builds a full account of what is wrong - which mods,
      which rule each one broke, which file to disable - handed it over as `extra`, and
      nothing ever read it. So a blocked launch told people "1 confirmed mod compatibility
      issue must be fixed before launch" and left them to guess which of a hundred mods it
      meant. Keeping it on lastLaunchFailure means it survives a status refresh and the
      panel can still show it after the window is reopened.
    */
    lastLaunchFailure = {
    ...
    emitStatus();
    // Every early exit from this handler goes through here, so one call covers all of
    // them: no token, no Java, a blocking update, a busy setup.
    reportLaunchMetric("failure", {
      mcVersion: readSettings().selectedVersion,
      reason: lastLaunchFailure.message
    });
...
async function launchStandaloneMinecraft(status, joinAddress = "") {
  ...
  // Preflight: resolve a REAL Java of the right major before spawning. Without this we'd
  // spawn the bare command "java", which dies with ENOENT on every machine that doesn't
  // have Java on PATH. Auto-install one if none exists. 26.x needs 25, not 21, and a
  // machine that already has 21 for the older versions still has to fetch it.
  const javaMajor = requiredJavaMajor(status.settings.selectedVersion);
  let javaPath = resolveUsableJava(status.settings, javaMajor);
  if (!javaPath) {
    emitActivity({ title: "Preparing Minecraft", detail: `Java ${javaMajor} not found. Installing it now...`, current: 0, total: 4 });
    emit("launcher:log", `[launcher] No Java ${javaMajor}+ found on this machine. Auto-installing Adoptium JRE ${javaMajor}.`);
    const install = await installJavaRuntime(javaMajor).catch((e) => ({ ok: false, message: e.message }));
    if (install.ok && install.javaPath) {
      javaPath = install.javaPath;
    } else {
      const msg = `River needs Java ${javaMajor} and it could not be installed automatically. Use Fix, or install Java ${javaMajor} (Adoptium Temurin) and launch again.`;
  ...
  try {
    ...
    setLaunchState("idle");
    emitStatus();
    // This one returns directly rather than through failLaunch, so it reports its own.
    reportLaunchMetric("failure", { mcVersion: status.settings.selectedVersion, reason: "java-spawn-failed" });
    return { ok: false, message: msg, fixable: true };
  }

  reportLaunchMetric("launch", { mcVersion: status.settings.selectedVersion });
...
async function announceSocialStatus(settings = readSettings()) {
  ...
  /*
    Every caller of this - the idle/active watchers and the status picker - goes down the
    open socket when there is one. Same meaning, no request, and it keeps all of them on
    one path rather than teaching each about the socket separately.
  */
  if (sendPresenceStatus(status)) return { ok: true, status };
  try {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), 6000);
...
/*
  Presence is a held socket rather than an announcement every 20 seconds.

  That interval was about 4,300 requests a day per person and, once the update poll
  became a push socket, easily the largest polling loop River had left. It also only ever
  approximated the thing it was measuring: the server had to assume somebody was gone if
  nothing arrived for 30 seconds, so closing River left a ghost in the roster for up to
  half a minute. A socket closing is the actual event, observed rather than inferred.

  The old POST is kept for two things it is still better at - a one-off status change,
  which is a single request either way, and as the fallback below for anyone whose
  network will not carry a WebSocket at all.
*/
const PRESENCE_WS_URL = "wss://updates.riverclient.xyz/presence/ws";
const PRESENCE_WS_PROTOCOL = "river.presence.v1";
const PRESENCE_PING_MS = 4 * 60 * 1000;
const PRESENCE_MAX_BACKOFF_MS = 2 * 60 * 1000;
/* Give the socket a fair few tries before falling back to announcing over HTTP. */
const PRESENCE_FALLBACK_AFTER = 3;

let statusHeartbeat = null;
let presenceSocket = null;
let presenceSocketPing = null;
let presenceSocketRetry = null;
let presenceSocketAttempts = 0;
...
function presenceHello() {
  ...
  const settings = readSettings();
  const status = SOCIAL_STATUS_VALUES.includes(settings.socialStatus) ? settings.socialStatus : "online";
  return { type: "hello", uuid: profile.id, name: profile.name, status };
}

/** Pushes a status change down the open socket. Falls back to the POST when there is none. */
function sendPresenceStatus(status) {
  if (presenceSocket && presenceSocket.readyState === 1) {
    try {
      presenceSocket.send(JSON.stringify({ type: "status", status }));
      return true;
    } catch {}
  }
  return false;
...
function stopPresenceSocket() {
  ...
  if (presenceSocketPing) { clearInterval(presenceSocketPing); presenceSocketPing = null; }
  if (presenceSocketRetry) { clearTimeout(presenceSocketRetry); presenceSocketRetry = null; }
  if (presenceSocket) {
...
/*
  The old announcement loop, started only when the socket cannot be established. Without
  it anyone behind a proxy that blocks WebSockets would simply stop appearing online.
*/
function startPresenceFallback() {
  if (statusHeartbeat || presenceStopped) return;
  statusHeartbeat = setInterval(() => { announceSocialStatus().catch(() => {}); }, 20000);
  announceSocialStatus().catch(() => {});
}

function startPresenceSocket() {
  if (typeof WebSocket !== "function") { startPresenceFallback(); return; }
  if (presenceSocket || presenceStopped) return;
  // Nothing to announce until somebody is signed in; connecting again is cheap.
  if (!presenceHello()) return;

  let socket;
  try {
    socket = new WebSocket(PRESENCE_WS_URL, PRESENCE_WS_PROTOCOL);
  } catch {
  ...
  socket.addEventListener("open", () => {
    presenceSocketAttempts = 0;
    // The socket carried no identity through the handshake, so say who this is now.
    const hello = presenceHello();
    if (hello) { try { socket.send(JSON.stringify(hello)); } catch {} }

    // The socket got through, so the fallback loop is not needed.
    if (statusHeartbeat) { clearInterval(statusHeartbeat); statusHeartbeat = null; }

    if (presenceSocketPing) clearInterval(presenceSocketPing);
    presenceSocketPing = setInterval(() => {
...
function schedulePresenceReconnect() {
  if (presenceStopped || presenceSocketRetry) return;
  presenceSocketAttempts += 1;
  if (presenceSocketAttempts >= PRESENCE_FALLBACK_AFTER) startPresenceFallback();
  const base = Math.min(5000 * 2 ** (presenceSocketAttempts - 1), PRESENCE_MAX_BACKOFF_MS);
  const wait = base / 2 + Math.random() * (base / 2);
  presenceSocketRetry = setTimeout(() => {
    presenceSocketRetry = null;
    startPresenceSocket();
  }, wait);
}

/*
  Direct messages reach the launcher, not just the game.

  Someone messaging you while River is open but Minecraft is not had no way of reaching
  you at all: the in-game client is the only thing that ever held a River social session,
  so the launcher did not know your friends existed.

  It signs in the same way the game does - the Mojang-signed player certificate, a
  one-time nonce from River, and a signature proving we hold the certificate's key. The
  launcher already stores the Minecraft access token that the certificate is fetched with,
  so this needs nothing new from the player.
*/
const SOCIAL_BASE = "https://updates.riverclient.xyz/social";
const SOCIAL_WS_URL = "wss://updates.riverclient.xyz/social/ws";
const SOCIAL_PROTOCOL = "river.social.v1";
const SOCIAL_PING_MS = 4 * 60 * 1000;
const SOCIAL_MAX_BACKOFF_MS = 2 * 60 * 1000;

let socialToken = "";
let socialSocket = null;
let socialSocketPing = null;
let socialSocketRetry = null;
let socialSocketAttempts = 0;
let socialStopped = false;
let socialSigningIn = false;

async function socialPost(route, body) {
  const response = await fetch(`${SOCIAL_BASE}${route}`, {
    method: "POST",
    headers: { "Content-Type": "application/json", "User-Agent": launcherUserAgent },
    body: JSON.stringify(body || {})
  });
...
/*
  Mojang labels these PEM blocks "RSA PRIVATE/PUBLIC KEY" but the bodies are PKCS#8 and
  SPKI, so the headers are stripped and the base64 used directly rather than trusting the
  label - the same thing the in-game client does with them.
*/
function stripPem(pem) {
  return String(pem || "").replace(/-----[^-]+-----/g, "").replace(/\s+/g, "");
}

async function fetchPlayerCertificate(accessToken) {
  try {
    ...
    if (!response.ok) return null;
    const payload = await response.json();
    const keyPair = payload?.keyPair;
    if (!keyPair?.publicKey || !keyPair?.privateKey) return null;
    return {
      publicKey: stripPem(keyPair.publicKey),
      privateKey: stripPem(keyPair.privateKey),
      // V2 covers uuid, expiry and key; the original signature left the uuid out.
      signature: String(payload.publicKeySignatureV2 || ""),
...
function signNonce(nonce, privateKeyBase64) {
  const key = crypto.createPrivateKey({
    key: Buffer.from(privateKeyBase64, "base64"),
    format: "der",
    type: "pkcs8"
  });
  return crypto.sign("sha256", Buffer.from(nonce, "utf8"), key).toString("base64");
}

async function ensureSocialSession() {
  if (socialToken || socialSigningIn) return socialToken;
  const auth = readAuth();
  if (!auth?.signedIn || !auth.minecraftAccessToken || !auth.profile?.id) return "";

  socialSigningIn = true;
  try {
    const begin = await socialPost("/auth/begin", {});
    const nonce = begin?.nonce;
    if (!nonce) return "";

    const certificate = await fetchPlayerCertificate(auth.minecraftAccessToken);
    if (!certificate?.signature) return "";

    const complete = await socialPost("/auth/complete", {
      name: auth.profile.name,
      uuid: auth.profile.id,
      nonce,
      ...
      nonceSignature: signNonce(nonce, certificate.privateKey)
    });
    if (complete?.ok !== true || !complete.token) return "";
    socialToken = String(complete.token);
    return socialToken;
  } catch {
    return "";
  } finally {
    socialSigningIn = false;
  }
}

function stopSocialSocket() {
  socialStopped = true;
  if (socialSocketPing) { clearInterval(socialSocketPing); socialSocketPing = null; }
  if (socialSocketRetry) { clearTimeout(socialSocketRetry); socialSocketRetry = null; }
  if (socialSocket) {
    try { socialSocket.close(); } catch {}
    socialSocket = null;
  }
  socialToken = "";
}

async function startSocialSocket() {
  if (typeof WebSocket !== "function" || socialSocket || socialStopped) return;
  const token = await ensureSocialSession();
  if (!token) return;

  let socket;
  try {
    /*
      The token rides as a subprotocol because Node cannot put it in a header, and the
      server accepts it there for exactly that reason. It is never put in the URL.
    */
    socket = new WebSocket(SOCIAL_WS_URL, [SOCIAL_PROTOCOL, `bearer.${token}`]);
  } catch {
  ...
  socket.addEventListener("open", () => {
    socialSocketAttempts = 0;
    if (socialSocketPing) clearInterval(socialSocketPing);
    socialSocketPing = setInterval(() => {
      try { socket.send("ping"); } catch {}
    }, SOCIAL_PING_MS);
  });
  ...
  socket.addEventListener("message", (event) => {
    ...
    if (payload?.type === "dm" && payload.message) notifyDirectMessage(payload.message);
  });

  const drop = () => {
    if (socialSocket === socket) socialSocket = null;
    if (socialSocketPing) { clearInterval(socialSocketPing); socialSocketPing = null; }
    /*
      A dropped socket can mean the session expired rather than the network blinking, and
      a stale token would fail the handshake forever. Clearing it costs one sign-in.
    */
    socialToken = "";
    scheduleSocialReconnect();
  };
...
function scheduleSocialReconnect() {
  if (socialStopped || socialSocketRetry) return;
  socialSocketAttempts += 1;
  const base = Math.min(5000 * 2 ** (socialSocketAttempts - 1), SOCIAL_MAX_BACKOFF_MS);
  const wait = base / 2 + Math.random() * (base / 2);
  socialSocketRetry = setTimeout(() => {
    socialSocketRetry = null;
    startSocialSocket().catch(() => {});
  }, wait);
}

/*
  A desktop notification when a message arrives.

  Skipped while the launcher is focused: you are looking at it, so a notification would
  be telling you something you can already see.

  The event is also emitted on the bus. Nothing listens for it yet - the renderer would
  need a channel in the preload to hear it - but the message is here and throwing it away
  to add it back later would be silly.
*/
function notifyDirectMessage(message) {
  const from = String(message.fromName || "Someone").slice(0, 32);
  const text = String(message.text || "").slice(0, 200);
  if (!text) return;

  emit("social:dm", { from, text, at: Number(message.at) || Date.now() });

  const settings = readSettings();
  if (settings.dmNotifications === false) return;
  /*
    Nothing from the launcher while the game is running. River is already showing the
    message in there, with its own ding and an on-screen popup, and a desktop notification
    on top of that is two alerts for one message - worse than either alone, and it steals
    focus from a game you are playing.
  */
  if (launchProcess) return;
  ...
  try {
    ...
    const notification = new Notification({
      ...
      body: text,
      icon: appIcon,
      // Windows plays its own message chime for this; the in-game client dings itself.
      silent: false
    });
...
function startStatusHeartbeat() {
  ...
  startPresenceSocket();
  startSocialSocket().catch(() => {});
  // One announcement now so the roster is right even before the socket finishes opening.
  announceSocialStatus().catch(() => {});
}
...
ipcMain.handle("launcher:get-partners", async () => {
  ...
  const results = (await mapWithConcurrency(partners, SERVER_STATUS_CONCURRENCY, fetchServerStatus))
...
/*
  River is told about a release instead of asking for one.

  Suggested by Jack (BGRD), who pointed out that a socket is the right shape for this and
  can carry everything else later. It is: an update now lands the moment it is published
  rather than up to a poll interval late, and the polling that used to answer "nothing has
  changed" hundreds of times a day per person is down to a fallback.

  The socket is deliberately anonymous. Node cannot set request headers on a WebSocket
  handshake, so it cannot carry the User-Agent and install id the usage counters read -
  which is fine, because the ordinary startup check above already reports those once per
  session. Nothing is counted here.
*/
const UPDATE_PUSH_URL = "wss://updates.riverclient.xyz/ws/updates";
const UPDATE_PUSH_PROTOCOL = "river.updates.v1";
const UPDATE_POLL_MS = 30 * 60 * 1000;
/* Long enough to stay out of the way, short enough to beat the usual idle proxy timeout. */
const UPDATE_PUSH_PING_MS = 4 * 60 * 1000;
const UPDATE_PUSH_MAX_BACKOFF_MS = 5 * 60 * 1000;

let updateSocket = null;
let updateSocketPing = null;
let updateSocketRetry = null;
let updateSocketAttempts = 0;
let updateSocketClosed = false;

function stopUpdatePushSocket() {
  updateSocketClosed = true;
  if (updateSocketPing) { clearInterval(updateSocketPing); updateSocketPing = null; }
  if (updateSocketRetry) { clearTimeout(updateSocketRetry); updateSocketRetry = null; }
  if (updateSocket) {
    try { updateSocket.close(); } catch {}
    updateSocket = null;
  }
}

function startUpdatePushSocket(onUpdate) {
  // Without a WebSocket the fallback interval is the whole mechanism, which still works.
  if (typeof WebSocket !== "function") return;
  if (updateSocket || updateSocketClosed) return;

  let socket;
  try {
    socket = new WebSocket(UPDATE_PUSH_URL, UPDATE_PUSH_PROTOCOL);
  } catch {
    scheduleUpdatePushReconnect(onUpdate);
    return;
  }
  updateSocket = socket;

  socket.addEventListener("open", () => {
    updateSocketAttempts = 0;
    if (updateSocketPing) clearInterval(updateSocketPing);
    /*
      Answered by Cloudflare without waking the object on the other end, so a keepalive
      costs nothing beyond the packet. Something has to be sent or an idle proxy will
      quietly drop the connection and neither side would notice.
    */
    updateSocketPing = setInterval(() => {
      try { socket.send("ping"); } catch {}
    }, UPDATE_PUSH_PING_MS);
  });
  ...
  socket.addEventListener("message", (event) => {
    ...
    /*
      "latest" is sent on connect and describes where things already stand, so it is only
      worth acting on when it disagrees with what we last saw - otherwise every reconnect
      would fire a redundant check. "update" means a release just shipped.
    */
    if (payload.type === "update") onUpdate();
    else if (payload.type === "latest" && payload.version && payload.version !== launcherUpdateState.latestVersion) onUpdate();
  });

  const drop = () => {
    if (updateSocket === socket) updateSocket = null;
    if (updateSocketPing) { clearInterval(updateSocketPing); updateSocketPing = null; }
    scheduleUpdatePushReconnect(onUpdate);
  };
  socket.addEventListener("close", drop);
...
/*
  Backs off so a server that is down does not get hammered by every launcher at once, and
  jitters so they do not all come back in the same second when it returns.
*/
function scheduleUpdatePushReconnect(onUpdate) {
  if (updateSocketClosed || updateSocketRetry) return;
  updateSocketAttempts += 1;
  const base = Math.min(5000 * 2 ** (updateSocketAttempts - 1), UPDATE_PUSH_MAX_BACKOFF_MS);
  const wait = base / 2 + Math.random() * (base / 2);
  updateSocketRetry = setTimeout(() => {
    updateSocketRetry = null;
    startUpdatePushSocket(onUpdate);
  }, wait);
}
...
function startLauncherUpdateWatcher() {
  ...
  /*
    The check on startup is the one that matters, and it is also the one the server
    counts - see the note on the socket below.

    The interval behind it is now only a safety net. River is told when a release lands
    rather than asking, so this exists for the case where the socket cannot be
    established at all: a proxy that blocks WebSockets, or a runtime without one. Half
    an hour is frequent enough to matter to somebody who leaves River open all day and
    rare enough to be invisible in the figures. It used to be every fifteen seconds,
    which filed nearly six thousand requests a day per person and buried the usage
    numbers under its own noise.
  */
  watch();
  updateWatcher = setInterval(watch, UPDATE_POLL_MS);
  startUpdatePushSocket(watch);
}
...
async function checkLauncherUpdates() {
  ...
  try {
    ...
    /*
      A newer version existing is not a reason to stop somebody playing. This used to be
      `available || required`, so every release locked everyone out of their own game until
      they had downloaded it - which for this launcher means several hundred megabytes
      before you can press Play.

      An update is now offered and can wait. The only thing that still blocks is a version
      that can no longer work: below the manifest's minimum, meaning its files are gone from
      the bucket and there is nothing left to run or repair from, or a release explicitly
      marked required because shipping it was the point.
    */
...
async function fetchRemoteUpdateManifest() {
  ...
  for (const url of candidates) {
    ...
    try {
      ...
      const response = await fetch(url, {
        signal: controller.signal,
        headers: metricsHeaders({ "Accept": "application/json" })
      });
...
async function installLauncherUpdate(forcedUpdate = null) {
  ...
  /*
    Anything the user could see an Install button for has to actually install.

    This used to check blocking, which was fine while blocking meant "there is an update".
    It now means "you are below the minimum and must update", so every optional update -
    which is nearly all of them - fell straight through this line and returned "already up
    to date" with ok true. The button rendered, did nothing, and said nothing, because
    nothing had technically failed.
  */
  if (!update.available && !update.blocking) {
    return { ok: true, message: "River Client is already up to date." };
  }
  ...
  if (differentialManifestUrl) {
    try {
      ...
      const filePlan = await buildDifferentialUpdatePlan(normalizedFileManifest, installDir);
...
/**
 * SHA-256 of a file, read in chunks, without blocking.
 *
 * The alternative is `createHash("sha256").update(fs.readFileSync(p))`, which loads the
 * whole file into memory before hashing any of it. That is a 213 MB allocation for
 * RiverClient.exe alone, and being synchronous it happens on the main process, where
 * nothing else - no window, no input, no IPC - runs until it finishes.
 */
...
/*
  Works out which files an update actually has to fetch.

  This checks every file the install is made of, and on a normal update almost all of them
  are unchanged, so almost all of them get hashed: about half a gigabyte across ninety odd
  files. It used to do that synchronously, so the launcher was frozen solid for the whole
  sweep - which is what "a bit laggy when you update" was. It now streams each file and
  yields between them, so the window keeps painting while it works.
*/
async function buildDifferentialUpdatePlan(fileManifest, installDir) {
  ...
  for (const entry of files) {
    ...
    // Size is checked first because it is a cheap stat and settles most changed files
    ...
    if (entry.sha256) {
      let localSha = "";
      try {
        localSha = await hashFileStreaming(localPath);
      } catch {
        // Unreadable means it cannot be verified, so fetch it rather than trust it.
        downloads.push(entry);
        continue;
      }
...
/*
  Rebuilds a changed file from the copy already on disk plus the few parts of it that
  actually differ.

  File level diffing already skips anything unchanged, which covers the Electron DLLs.
  It does nothing for the two files that change on every single build - RiverClient.exe,
  because the ASAR integrity hash and the signature are written into it, and app.asar,
  because it holds our own JavaScript. Those two are 321MB together and almost none of
  either is different, so every update re-downloaded a third of a gigabyte to change a
  couple of megabytes.

  The server publishes a blockmap beside each big file: the content-defined chunks it is
  made of, and a hash for each. Chunk the local copy the same way and most of those
  hashes are already on disk, so only the rest has to come over the network, as a handful
  of range requests.

  Everything here is best effort. The rebuilt file is checked against the whole-file hash
  from the release manifest, and anything at all that does not line up - no blockmap, an
  algorithm this build does not know, a bad range response, a hash that does not match -
  returns false and lets the caller download the file the ordinary way. It can make an
  update smaller; it must never make one wrong.
*/
async function downloadFileViaBlockmap(file, localPath, target, onProgress) {
  if (!file.blockmapUrl || !file.sha256 || !localPath || !fs.existsSync(localPath)) return false;

  let map;
  try {
    const response = await fetch(file.blockmapUrl, { headers: { "User-Agent": launcherUserAgent } });
    if (!response.ok) return false;
    map = await response.json();
  } catch {
    return false;
  }
  if (!map || map.algorithm !== "gear-cdc-1" || !Array.isArray(map.chunks) || !map.chunks.length) return false;
  if (Number(map.size) !== Number(file.size)) return false;

  // Index the local copy by chunk hash. The bytes are dropped straight after; only the
  // offsets are kept, and the ranges are read back off disk when they are needed.
  const local = new Map();
  try {
    const buffer = fs.readFileSync(localPath);
    let offset = 0;
    for (const chunk of chunkBuffer(buffer)) {
      if (!local.has(chunk.hash)) local.set(chunk.hash, { offset, size: chunk.size });
      offset += chunk.size;
    }
  } catch {
    // Too big to hold, or unreadable. Either way this is not the path to take.
    return false;
  }

  // Work out what comes from disk and what has to be fetched, coalescing neighbouring
  // missing chunks so this is a few range requests rather than one per chunk.
  const steps = [];
  let position = 0;
  let missing = 0;
  for (const chunk of map.chunks) {
    ...
    const hit = local.get(chunk.hash);
    if (hit && hit.size === size) {
      steps.push({ kind: "local", at: position, from: hit.offset, size });
    } else {
      const last = steps[steps.length - 1];
      if (last && last.kind === "remote" && last.at + last.size === position) last.size += size;
      else steps.push({ kind: "remote", at: position, size });
      missing += size;
    }
    position += size;
  }
  if (position !== Number(file.size)) return false;

  // Nothing to gain if most of it has to come anyway.
  if (missing > Number(file.size) * 0.75) return false;

  emit("launcher:log", `[update] ${file.path}: ${formatBytes(missing)} of ${formatBytes(file.size)} changed`);

  let readFd = null;
  let writeFd = null;
  try {
    readFd = fs.openSync(localPath, "r");
    writeFd = fs.openSync(target, "w");
    let done = 0;
    for (const step of steps) {
      if (step.kind === "local") {
        const buffer = Buffer.allocUnsafe(step.size);
        fs.readSync(readFd, buffer, 0, step.size, step.from);
        fs.writeSync(writeFd, buffer, 0, step.size, step.at);
      } else {
        const response = await fetch(file.url, {
          headers: {
            "User-Agent": launcherUserAgent,
            Range: `bytes=${step.at}-${step.at + step.size - 1}`
          }
        });
        // A server that ignored the range would hand back the whole file, which would be
        // written to the wrong place. Only a 206 means the range was honoured.
        if (response.status !== 206) return false;
        const buffer = Buffer.from(await response.arrayBuffer());
        if (buffer.length !== step.size) return false;
        fs.writeSync(writeFd, buffer, 0, buffer.length, step.at);
        done += buffer.length;
        if (onProgress) onProgress({ downloaded: done, total: missing });
      }
    }
  } catch {
    return false;
  } finally {
    if (readFd !== null) { try { fs.closeSync(readFd); } catch {} }
    if (writeFd !== null) { try { fs.closeSync(writeFd); } catch {} }
  }

  // The whole point of the exercise is that this cannot be taken on trust.
  let rebuilt = "";
  try {
    rebuilt = await hashFileStreaming(target);
  } catch {
    return false;
  }
  if (rebuilt !== file.sha256) {
    emit("launcher:log", `[update] ${file.path}: rebuilt copy did not match, downloading it in full`);
    try { fs.rmSync(target, { force: true }); } catch {}
    return false;
  }

  emit("launcher:log", `[update] ${file.path}: rebuilt from disk, ${formatBytes(missing)} downloaded`);
  return true;
}
...
async function downloadChangedUpdateFiles(plan, extractDir, targetVersion) {
  ...
  for (let index = 0; index < plan.downloads.length; index += 1) {
    ...
    /*
      Try to rebuild it from the copy already installed first. Returns false for any
      reason at all - no blockmap, a bad range, a hash that does not match - and the
      ordinary download below then runs exactly as it always did.
    */
    const localPath = plan.installDir ? path.join(plan.installDir, ...file.path.split("/")) : "";
    const patched = await downloadFileViaBlockmap(file, localPath, target, ({ downloaded, total }) => {
      emitActivity({
        title: "Updating River Client",
        detail: `Patching update files (${index + 1} of ${plan.downloads.length})`,
        current: downloadedBase + downloaded,
        total: plan.totalBytes || total || 0,
        unit: "bytes",
...
async function prepareStandaloneMinecraftLaunch(status, auth, authenticated, joinAddress = "") {
  ...
  // Frame rate settings are written now rather than at save time, so an instance that
  // was imported or edited in game still ends up matching what River is showing.
  const fpsChanges = applyFrameRateOptions(status.instancePath, {
    maxFps: status.settings.maxFps,
    vsync: status.settings.vsync
  });
  for (const change of fpsChanges) emit("launcher:log", `[launcher] options.txt ${change}`);
...
async function prepareAssets(assetIndex, assetsDir) {
  ...
  /*
    Once an index has been fully fetched, record that against the index's own sha1.
    Nothing about a completed asset set changes between launches - the objects are
    content-addressed and the index is immutable - so re-testing every one of the
    ~5000 files costs over a second of startup to always conclude "all present".

    The stamp is only trusted as far as it can be cheaply falsified: a sample of the
    objects is still checked, which catches the cases that actually happen (the folder
    cleared out, a drive that came back empty) without walking the whole set. Anything
    subtler is caught by the game itself, which re-requests an asset it cannot read.
  */
  const stampPath = path.join(indexesDir, `${assetIndex.id || "assets"}.complete.json`);
  const stampMatches = () => {
    try {
      const stamp = JSON.parse(fs.readFileSync(stampPath, "utf8"));
      if (stamp.sha1 !== assetIndex.sha1 || stamp.count !== objects.length) return false;
      const step = Math.max(1, Math.floor(objects.length / 24));
      for (let i = 0; i < objects.length; i += step) {
        const object = objects[i];
        if (!fs.existsSync(path.join(objectsDir, object.hash.slice(0, 2), object.hash))) return false;
      }
      return true;
    } catch {
      return false;
  ...
  if (objects.length && stampMatches()) {
    emitActivity({
      title: "Preparing assets",
      detail: `${objects.length} / ${objects.length} assets ready.`,
  ...
  // Only written after every object came back present or downloaded, so a run that
  // failed part way through is retried in full next launch rather than stamped.
  try {
    fs.writeFileSync(stampPath, JSON.stringify({ sha1: assetIndex.sha1, count: objects.length, at: Date.now() }));
  } catch {}
...
app.on("will-quit", () => {
  ...
  // Otherwise the reconnect timers keep the process alive after the windows are gone.
  stopUpdatePushSocket();
  stopPresenceSocket();
  stopSocialSocket();
});
