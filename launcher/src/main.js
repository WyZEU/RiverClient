...
function startLauncherUpdateWatcher() {
  ...
  /*
    Five minutes, not the fifteen seconds this used to poll at.

    Every one of these is a request the server counts as an update check, so a launcher
    left open all day filed nearly six thousand of them and buried the usage figures
    under its own noise - the version chart read in the thousands off a handful of
    people. Nothing needed that cadence: an update that arrives while River is sitting
    open can wait five minutes to be noticed, and the check on startup just above is
    what catches it for anyone actually about to play.
  */
  watch();
  updateWatcher = setInterval(watch, 5 * 60 * 1000);
}
