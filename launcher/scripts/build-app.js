...
async function bundle() {
  ...
  // The main window's bridge, plus one each for the updater and log windows. Those two
  // are separate files rather than one shared bridge so each window is handed only the
  // channels it uses.
  for (const name of ["preload.js", "preload-updater.js", "preload-logs.js"]) {
    await esbuild.build({
      entryPoints: [path.join(root, "src", name)],
      bundle: true,
      platform: "node",
      target: "node22",
      minify: false,
  ...
  /*
    Both of these windows are bundled locally and talk to the main process over their
    preload bridge, so nothing they need comes off the network. Saying that explicitly
    means a bug in a dependency cannot turn into a request leaving the machine.
    'unsafe-inline' is there for styles only, because React sets style attributes.
  */
  ...
  /*
    The window paints a frame before its bundle parses and React mounts. Without this that
    frame is a blank dark rectangle. This static markup is styled inline so it is on screen
    on the first frame; each entry removes #boot right after it renders (see updater.jsx /
    logs.jsx). It is a sibling of #root, not a child, so React never sees a non-empty root.
  */
  const splash = (label) =>
    "<div id=\"boot\" role=\"status\" style=\"position:fixed;inset:0;z-index:9999;"
    + "display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;"
    + "background:#0e1218;color:#eef0f4;font-family:'Segoe UI',Inter,system-ui,sans-serif;"
    + "-webkit-app-region:drag\">"
    + "<strong style=\"font-size:13px;font-weight:600;letter-spacing:.02em\">River Client</strong>"
    + "<span style=\"font-size:11px;color:#9297a2\">" + label + "</span>"
    + "<div style=\"width:150px;height:3px;border-radius:999px;background:#23262e;overflow:hidden\">"
    + "<div style=\"width:40%;height:100%;border-radius:inherit;background:#3b82f6;"
    + "animation:river-splash 1.15s ease-in-out infinite\"></div></div>"
    + "<style>@keyframes river-splash{0%{transform:translateX(-100%)}100%{transform:translateX(325%)}}</style>"
    + "</div>";
  const shell = (title, name, splashLabel) =>
    "<!doctype html><html lang=\"en\"><head><meta charset=\"UTF-8\" />"
    + "<meta http-equiv=\"Content-Security-Policy\" content=\"" + csp + "\" />"
    + "<meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\" />"
    + "<title>" + title + "</title>"
    + "<link rel=\"stylesheet\" href=\"./" + name + ".css\" /></head>"
    + "<body>" + splash(splashLabel) + "<div id=\"root\"></div>"
    + "<script src=\"./" + name + ".js\"></script></body></html>";

  fs.writeFileSync(path.join(out, "renderer", "updater.html"), shell("River Client Updater", "updater", "Preparing update"));
  fs.writeFileSync(path.join(out, "renderer", "logs.html"), shell("River Client - Game log", "logs", "Loading log"));
}
