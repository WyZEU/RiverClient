...
const { buildBlockmap, BLOCKMAP_MIN_BYTES } = require("./blockmap.js");
...
/*
  One instant for the whole publish, so the manifest and the Discord post cannot disagree
  about when the release happened. The messages file is written well before the manifest
  is built, so taking the time separately in each place would give two different answers
  for the same release.
*/
const publishedAt = new Date().toISOString();
const publishedAtUnix = Math.floor(new Date(publishedAt).getTime() / 1000);
...
/**
 * Orders River's four-part versions, so 0.1.8.10 sorts after 0.1.8.9 rather than before it.
 * Returns a negative number when [left] is older.
 */
...
function buildReleaseFileManifest(unpackedRoot, workerUrl, releaseKey) {
  ...
  const files = collectFilesRecursive(unpackedRoot).map((fullPath) => {
    ...
    /*
      Big files get a blockmap so an update can fetch only the parts of them that changed.

      Two files here are ever worth it, but they are the two that matter: the executable
      and app.asar are 321MB between them and change on every single build, while almost
      none of their contents actually differ. Measured across two real consecutive builds,
      a blockmap takes those two from 321MB down to about 1.7MB.
    */
    const blockmapUrl = size >= BLOCKMAP_MIN_BYTES
      ? baseUrl + "/" + relative.split("/").map(encodeURIComponent).join("/") + ".blockmap.json"
      : "";
    return {
      path: relative,
      size,
...
function createAppPackage(paths) {
  ...
  /*
    Zipping is retried, because it competes with whatever is still reading the files
    electron-builder has only just written.

    Compress-Archive opens each file without allowing anyone else to have it open, so a
    virus scanner working through a freshly built 500 MB tree is enough to fail the whole
    step - which it did on four consecutive releases, landing on app.asar one time,
    dxcompiler.dll the next, whichever file was being read at that moment. The scan is
    over in seconds, so waiting and trying again is the fix. Failing outright and making
    a person rerun a ten minute build is not.
  */
  const attempts = 4;
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    ...
    console.log(`[publish] zip failed, a file was still in use. Retrying in ${waitSeconds}s (${attempt}/${attempts - 1})`);
    // Synchronous on purpose: this runs inside a linear publish, and there is nothing
    // else that should be happening while it waits.
    Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, waitSeconds * 1000);
  }
  throw new Error(`App package missing after ${attempts} attempts: ${appPackage}. A file in the build was locked by another process the whole time.`);
}
...
function syncSiteAssets() {
  ...
  /*
    partners.json is no longer synced from riv3r-site either, for exactly the reason the
    screenshots were removed above: the legacy copy is stale and this quietly overwrote the
    current one on every publish. A partner was removed from riv3r-next, committed, and
    deployed - and then reappeared on the site twice, because each release copied the dead
    folder's version back over the top. riv3r-next owns the partner list.
  */
  const assetTargets = [
...
function validateReleaseDraft(draft) {
  ...
  for (const key of ["announcement", "release"]) {
...
function writeReleaseMessages(draft) {
  ...
  /*
    Discord renders <t:unix:f> in each reader's own timezone, so the post shows the right
    local time for everybody rather than whatever the clock said on this machine. Emitted
    here rather than typed by hand because it has to match the manifest, and converting a
    timestamp by eye every release is the kind of thing that quietly goes wrong.
  */
  const stamp = `<t:${publishedAtUnix}:f>`;
  const text = sections
...
/**
 * Clears the Cloudflare edge cache for riverclient.xyz.
 *
 * Without this a Pages deploy succeeds and the live site still serves the
 * previous build, which reads as "the deploy did nothing" and sends you looking
 * for a bug that is not there.
 *
 * Never throws: a publish that shipped the app but could not purge is still a
 * successful publish, it just needs a manual purge afterwards.
 */
/*
  Tells every running launcher that the release is live, now that it actually is.

  This is the server half of replacing the update poll: without it the sockets sit there
  and nobody hears anything until the fallback interval comes round, which would make the
  whole exercise pointless. It runs last on purpose - announcing before the manifest is
  in place would send launchers after a version the manifest does not yet describe.

  A failure here is logged and swallowed. Push is an optimisation on top of a fallback
  that still works, so a release should not fail because a notification did not go out.
*/
async function announceRelease({ workerUrl, minimumVersion, beta }) {
  if (beta) {
    console.log("[publish] beta channel: not announcing to stable launchers");
    return;
  }
  const done = logStep("announce the release to connected launchers");
  try {
    let key = process.env.REFERRAL_STATS_KEY || "";
    if (!key) {
      for (const root of [launcherRoot, repoRoot]) {
        for (const name of ["secrets.env", "secrects.env"]) {
          const file = path.join(root, name);
          if (!fs.existsSync(file)) continue;
          const found = fs.readFileSync(file, "utf8").match(/^REFERRAL_STATS_KEY=(.+)$/m);
          if (found) { key = found[1].trim().replace(/^["']|["']$/g, ""); break; }
        }
        if (key) break;
      }
    }
    if (!key) {
      console.warn("[publish] no REFERRAL_STATS_KEY, skipping the release announcement");
      done();
      return;
    }

    const endpoint = String(workerUrl).replace(/[/]+$/, "") + "/admin/announce";
    const response = await fetch(endpoint, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${key}`,
        "Origin": "https://river-stats.pages.dev",
        "User-Agent": "river-publish"
      },
      body: JSON.stringify({ version, minimumVersion, publishedAt })
    });
    const result = await response.json().catch(() => null);
    if (!response.ok || !result?.ok) {
      console.warn(`[publish] announcement failed (${response.status}), launchers will fall back to polling`);
    } else {
      console.log(`[publish] announced ${version} to ${result.sent} connected launcher(s)`);
    }
  } catch (error) {
    console.warn(`[publish] announcement failed: ${error?.message || error}`);
  }
  done();
}

async function purgeSiteCache(zoneName = "riverclient.xyz") {
  const done = logStep(`purge Cloudflare cache for ${zoneName}`);
  try {
    const token = process.env.CLOUDFLARE_API_TOKEN;
    const key = process.env.CLOUDFLARE_API_KEY;
    const email = process.env.CLOUDFLARE_EMAIL;
    const headers = { "Content-Type": "application/json", "User-Agent": "river-publish" };
    if (token) headers.Authorization = `Bearer ${token}`;
    else if (key && email) { headers["X-Auth-Key"] = key; headers["X-Auth-Email"] = email; }
    else { console.warn("[publish] no Cloudflare auth for cache purge, skipping"); done(); return; }
    ...
    if (!zoneId) {
      console.warn(`[publish] could not find the ${zoneName} zone, skipping purge`);
      done();
      return;
    }

    const purged = await call(`/zones/${zoneId}/purge_cache`, {
      method: "POST",
      body: JSON.stringify({ purge_everything: true })
    });
    if (!purged?.success) {
      console.warn(`[publish] cache purge failed: ${JSON.stringify(purged?.errors || purged).slice(0, 200)}`);
      console.warn("[publish] the site is deployed but visitors may see the old build until the cache expires.");
    }
  } catch (error) {
    console.warn(`[publish] cache purge errored: ${error.message}`);
  }
  done();
}
...
async function main() {
  ...
  const blockmapDir = path.join(launcherRoot, "dist", "blockmaps");
  fs.rmSync(blockmapDir, { recursive: true, force: true });
  fs.mkdirSync(blockmapDir, { recursive: true });

  for (const entry of fileManifest.files) {
    const fullPath = path.join(paths.unpacked, ...entry.path.split("/"));
    if (entry.blockmapUrl) {
      const localMap = path.join(blockmapDir, entry.path.replace(/[\/]/g, "_") + ".blockmap.json");
      writeJson(localMap, buildBlockmap(fullPath));
      uploadTasks.push({
        label: `app/${entry.path}.blockmap.json`,
        args: ["r2", "object", "put", `${bucket}/${releaseKey}/app/${entry.path}.blockmap.json`, "--file", localMap, "--content-type", "application/json", "--remote"]
      });
    }
    uploadTasks.push({
  ...
  /*
    The oldest release people are still allowed to run, which is a completely different
    question from what the newest one is.

    This used to be the version being published, so every single release put every existing
    install below the minimum and forced it to update before it could launch. Updating is
    now something you are offered; the minimum is the line where an install genuinely
    cannot keep going, because its files have been deleted from the bucket and there is
    nothing left to run or repair from.

    Raise RIVER_MINIMUM_VERSION when release files are actually deleted, not when a new
    release goes out.
  */
  const minimumVersion = String(process.env.RIVER_MINIMUM_VERSION || "0.1.7").trim();
  if (compareReleaseVersions(minimumVersion, version) > 0) {
    throw new Error(`RIVER_MINIMUM_VERSION (${minimumVersion}) is newer than the version being published (${version}).`);
  }
  ...
  if (flags.beta) {
    ...
    donePages();
    // Pages deploys fine but riverclient.xyz keeps serving the old HTML from
    // Cloudflare's edge cache, so a publish looks like it did nothing. This has
    // caught me twice now - once with screenshots, once with the whole homepage.
    await purgeSiteCache();
  } else {
    console.log("[publish] skipping Cloudflare Pages deploy");
  }

  await announceRelease({ workerUrl, minimumVersion, beta: flags.beta });
