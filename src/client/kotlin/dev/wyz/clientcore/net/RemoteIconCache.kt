...
object RemoteIconCache {
    ...
    data class Icon(val id: Identifier, val width: Int, val height: Int)
    ...
    private fun load(url: String, state: State) {
        pool.submit {
            ...
            Minecraft.getInstance().execute {
                runCatching {
                    val id = Identifier.fromNamespaceAndPath("clientcore", "content_icon_${counter.incrementAndGet()}")
                    Minecraft.getInstance().textureManager.register(id, DynamicTexture({ "river-content-icon" }, image))
