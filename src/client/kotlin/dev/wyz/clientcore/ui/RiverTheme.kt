...
object RiverTheme {
    ...
    /*
      The default is the blue from riverclient.xyz (hsl(217 80% 58%) = #3E80EA), so the
      client, the launcher and the site are the same colour. It used to be indigo to purple,
      which meant the one thing every new user saw first did not match River anywhere else.

      Its two stops sit inside a single hue family on purpose. The gradient exists so the
      other presets and a custom pick can use it, but the default should read as one blue
      rather than as a colour sweep.
    */
    val presets: List<Palette> = listOf(
        Palette("river", "River", 0xFF3E80EA.toInt(), 0xFF5A9BF5.toInt(), 0xFF7FB4FF.toInt()),
