...
object ClientUi {
    ...
    /**
     * Corner radii. Minecraft's GUI is square and Essential only softens its corners a
     * little, so these stay tight - the old 12-14px radii read as a web app floating in
     * front of the world.
     */
    const val RADIUS_PANEL = 6
    const val RADIUS_CARD = 4
    const val RADIUS_BUTTON = 4
    ...
    fun drawPanel(g: GuiGraphics, x: Int, y: Int, w: Int, h: Int, radius: Int = RADIUS_PANEL) {
        drawShadow(g, x, y, w, h, radius + 2, alpha(0xFF000000.toInt(), 0.45f), 3)
    ...
    private fun drawTexturedToggle(g: GuiGraphics, key: String, x: Int, y: Int, trackW: Int, trackH: Int, knobSize: Int, on: Boolean, hovered: Boolean) {
        ...
        if (track != null && knob != null) {
            g.blit(net.minecraft.client.renderer.RenderPipelines.GUI_TEXTURED, track, x, y, 0f, 0f, trackW, trackH, 136, 56, 136, 56, trackTint)
            g.blit(net.minecraft.client.renderer.RenderPipelines.GUI_TEXTURED, knob, knobX, knobY, 0f, 0f, knobSize, knobSize, 48, 48, 48, 48, 0xFFFFFFFF.toInt())
        } else {
    ...
    fun drawShadow(g: GuiGraphics, x: Int, y: Int, w: Int, h: Int, radius: Int, color: Int, spread: Int) {
        val limitedSpread = spread.coerceIn(1, 3)
        for (i in limitedSpread downTo 1) {
            val alpha = i.toFloat() / limitedSpread.toFloat() * 0.38f
            fillRounded(g, x - i, y - i, w + i * 2, h + i * 2, radius + i, this.alpha(color, alpha))
        }
    }
