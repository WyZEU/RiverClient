...
object WatermarkRenderer {
    ...
    private fun logo(): Identifier? {
        if (!logoLoadAttempted) {
            ...
            logoTexture = runCatching {
                ...
                val id = Identifier.fromNamespaceAndPath("clientcore", "river_watermark_dynamic")
                Minecraft.getInstance().textureManager.register(id, DynamicTexture({ "river-watermark" }, image))
                id
            }.getOrNull()
        }
        return logoTexture
    ...
    fun renderOverlay(client: Minecraft, g: GuiGraphics) {
        ...
        if (hasLogo) {
            ...
            g.blit(RenderPipelines.GUI_TEXTURED, logo()!!, x, logoY, 0f, 0f, logoW, LOGO_H, TEX_W, TEX_H, TEX_W, TEX_H)
        }
