...
class NowPlayingModule : Module("now_playing", "Spotify", "Now playing from your music", ModuleCategory.HUD, "spotify", 8, 244) {
    ...
    private fun logo(): Identifier? {
        if (!logoTried) {
            ...
            logoTexture = runCatching {
                ...
                val id = Identifier.fromNamespaceAndPath("clientcore", "spotify_logo_dynamic")
    ...
    private fun cover(artFile: String): Identifier? {
        ...
        return runCatching {
            ...
            val id = Identifier.fromNamespaceAndPath("clientcore", "spotify_cover_dynamic")
    ...
    private fun draw(client: Minecraft, graphics: GuiGraphics, t: Track) {
        ...
        if (showArt) {
            if (art != null) {
                graphics.blit(RenderPipelines.GUI_TEXTURED, art, x, y, 0f, 0f, COVER, COVER, coverW, coverH, coverW, coverH)
            } else {
                ClientUi.fillRounded(graphics, x, y, COVER, COVER, ClientUi.RADIUS_CARD, ClientUi.PANEL_ALT)
                logo()?.let { graphics.blit(RenderPipelines.GUI_TEXTURED, it, x + 6, y + 6, 0f, 0f, COVER - 12, COVER - 12, 64, 64, 64, 64) }
            }
        }
