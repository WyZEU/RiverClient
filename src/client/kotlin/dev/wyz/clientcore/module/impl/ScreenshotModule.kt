...
class ScreenshotModule : Module("screenshot", "Screenshot", "F9 to capture your screen", ModuleCategory.UTILITY, "camera", 8, 262) {
    ...
    /*
      26.1 took the main render target off Minecraft along with the rest of the
      framebuffer plumbing, so the overload that accepts one cannot be reached there any
      more. Screenshot.grab(minecraft, ...) is the path vanilla itself uses now: it
      writes the file and posts its own saved-to message, which is why the 26.x arm has
      no callback and no River prefix on that line.
    */
